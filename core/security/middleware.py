...
class SecurityMiddleware(BaseHTTPMiddleware):
    ...
    async def _validate_request(self, request: Request, client_ip: str):
        ...
        # IDS (Intrusion Detection System) analysis
        ids_result = ids.analyze_request(request)
        threats = ids_result["threats"]
        
        if threats:
            from core.database.database import SessionLocal
            from db.models import SecurityEvent, ThreatType, SecurityLevel
            ...
            max_threat = max(threats, key=lambda t: severity_map.get(t["severity"], SecurityLevel.LOW))
            max_lvl = severity_map.get(max_threat["severity"], SecurityLevel.MEDIUM)
            
            threat_type_map = {
                "SQL Injection": ThreatType.BRUTE_FORCE,
                "XSS Attempt": ThreatType.POLICY_VIOLATION,
                "Path Traversal": ThreatType.POLICY_VIOLATION,
                "Command Injection": ThreatType.POLICY_VIOLATION
            }
            ttype = threat_type_map.get(max_threat["type"], ThreatType.POLICY_VIOLATION)
                ...
                try:
                    async with SessionLocal() as db:
                        event = SecurityEvent(
                            ...
                            description=f"Intrusion attempt blocked: {len(threats)} threat signature(s) detected.",
                            details={"threats": threats, "risk_score": ids_result["risk_score"]},
            ...
            asyncio.create_task(log_threat())
            
            if ids_result["risk_score"] >= 5:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
